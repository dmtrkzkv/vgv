...
void card::RefreshBitmap() { //oeffnet bild
	...
	bmp.read("../data/card_images_new/h4.2.pnm");
}
...
void card::loadAllImages() {//habe es im main versetzt!
	string file;
	for (int i = 0; i < 104; i++) {
...
double card::getAngle(double avAngle, point2d origin, point2d its1, point2d its2, point2d its3, point2d its4) { //DIMA
	point2d ref;
...
double card::averageAngle(point2d its1, point2d its2, point2d its3, point2d its4) { //DIMA
	double dx, dy;
 ...
 */
point2d card::detectBegin(int frame, bitmap bmp, point2d begin) {//DIMA
...
void card::Demo() {
	...
	int it = 0;
		...
		//double averageAngle = averageAngle(its1, its2, its3, its4);
		//double angle = getAngle(averageAngle, origin, its1, its2, its3, its4);
		double length = getLength(its1, its2, its4);

		//bitmap* newBmp = new bitmap(6, 300, 200, 255);
		bitmap* newBmp = new bitmap(6, 110, 100, 255); //mit dem modifizierten Scale in rotateBmp bekommt man nun nur den Wert
		rotateBmp(newBmp,origin,angle,length);
		blackWhite(newBmp);
	...
	//bmp.write("./copy.pnm");
	//newBmp->write("./newBmp.pnm");
	//system("eog ./newBmp.pnm");

	newBmp->write("../data//newBmp.pnm");
	system("eog ./newBmp.pnm");
